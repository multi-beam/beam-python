...
#   return_h (bool) Flag to return a matrix of scalar forward solutions.
#       Default is False
#
...
#   h_mtx   (n_chan x n_src) numpy array of the scalar FS; it is
#       only returned when the function is called with return_h = True
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
    ...
    return (w_mtx, u, h_mtx) if return_h else (w_mtx, u)
...
# -------------------------------------------------------------------------
    if len(fs.shape) == 3:
        ...
        for i in range(n_src):
            ...
            res = construct_mcmv_weights(f, r_inv, n_cov, beam, c_avg, return_h)
            w_mtx[:,i] = res[0][:,0]
            u[:,i] = res[1][:,0]
